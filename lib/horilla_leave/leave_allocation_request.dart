...
class _LeaveAllocationRequest extends State<LeaveAllocationRequest>
  ...
  // Removed artificial delay - loading is now controlled by actual data fetching
  ...
  void _scrollListener() {
    if (_scrollController.offset >=
            _scrollController.position.maxScrollExtent -
                200 && // Load more before reaching exact bottom
        !_scrollController.position.outOfRange &&
        !_isLoadingMore) {
      if (allocationCheck && _tabController.index == 0 && _hasMoreAllRequests) {
        _allRequestsPage++;
        getAllocationRequest();
      } else if ((!allocationCheck || _tabController.index == 1) &&
          _hasMoreMyRequests) {
        _myRequestsPage++;
        getMyAllocationRequest();
      }
...
''';
  ...
  Widget _buildLoadingWidget() {
    ...
    return Column(
      children: [
        ...
        SizedBox(height: MediaQuery.of(context).size.height * 0.02),
        TabBar(
          labelColor: const Color(0xFF6B57F0),
          indicatorColor: const Color(0xFF6B57F0),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return buildMyLeaveTiles(record, fullName, baseUrl, getToken,
                  profile ?? "", stateInfo);
            },
          ),
        ),
  ...
  List<Map<String, dynamic>> _filterRecords(
      List<Map<String, dynamic>> records, String query) {
    ...
    return records.where((record) {
      final employeeName =
          record['employee_id']['full_name']?.toString().toLowerCase() ?? '';
      final leaveType =
          record['leave_type_id']['name']?.toString().toLowerCase() ?? '';
      final status = record['status']?.toString().toLowerCase() ?? '';
      final requestedDays =
          record['requested_days']?.toString().toLowerCase() ?? '';
  ...
  Widget buildMyLeaveTiles(Map<String, dynamic> record, fullName, baseUrl,
      token, String profile, stateInfo) {
  ...
  Widget buildAllLeaveTile(Map<String, dynamic> record, fullName, baseUrl,
      token, String profile, stateInfo, String recordId) {
    return GestureDetector(
      onTap: () async {
...
StateInfo _getStateInfo(String state) {
  switch (state) {
    ...
    case 'cancelled':
      return StateInfo(const Color(0xFF6B57F0), 'Cancelled');
    case 'rejected':
